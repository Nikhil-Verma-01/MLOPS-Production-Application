...
def outlier_detection_step(df: pd.DataFrame, column_name: str) -> pd.DataFrame:
    ...
    if df is None:
        logging.error("Recevied a NoneType DataFrame.")
    ...
    oultier_detector = OutlierDetector(ZScoreOutlierDetection(threshold=3))
    oultiers = oultier_detector.detect_outliers(df_numeric)
    df_cleaned = oultier_detector.handle_oultiers(df_numeric, method="remove")
    return df_cleaned
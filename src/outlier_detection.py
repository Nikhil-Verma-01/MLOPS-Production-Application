...
class ZScoreOutlierDetection(OutlierDetectionStrategy):
    ...
    def detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame: # type: ignore
        logging.info("Detetcion outliers using the Z-score method.")
        z_scores = np.abs((df - df.mean()) / df.std())
        oultiers = z_scores > self.threshold
        logging.info(f"Outliers detected with Z-scores threshold: {self.threshold}")
        return oultiers # type: ignore
    

class IQROutlierDetection(OutlierDetectionStrategy):
    def detect_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        logging.info("Detetcing the outliers using the QIR method")
        Q1 = df.quantile(0.25)
        Q3 = df.quantile(0.75)
        IQR = Q3 - Q1
...
class OutlierDetector:
    ...
    def handle_oultiers(self, df: pd.DataFrame, method="remove", **kwargs) -> pd.DataFrame:
        outliers = self.detect_outliers(df)
        if method == "remove":
            logging.info("Removing oultiers from the dataset.")
    ...
    def viualize_outliers(self, df: pd.DataFrame, features: list):
        ...
        for feature in features:
            ...
            plt.show()
        logging.info("Outlier visualisationb completed.")
